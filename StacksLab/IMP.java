...
class IMP implements MouseListener{
   ...
   */  
    private void fun2()
    {
        Stack<PixelRegion> pixelStack = new Stack();//Stack of pixels that match
        pixelStack.push(new PixelRegion(colorX, colorY));//Original Pixel
      
        int rgbArray[] = new int[4];
        rgbArray = getPixelArray(picture[colorY][colorX]);
        int rgb = getPixels(rgbArray);
      
        int black[] = {255, 0, 0, 0};
      
        while(pixelStack.empty() == false){
          
            PixelRegion currentPix = pixelStack.peek();
            System.out.println(pixelStack.peek().getX()+" "+pixelStack.peek().getY());
            int colorArray[] = new int[4];
            int color;
                                    
            //check pixel to top left
            if(currentPix.checkAdjPix(0) == false)
            {                
                colorArray = getPixelArray(picture[currentPix.getY()-1][currentPix.getX()-1]);
                color = getPixels(colorArray);
                currentPix.setAdjPix(0, true);
                if(color == rgb)
                {
                    picture[currentPix.getY()-1][currentPix.getX()-1] = getPixels(black);
                    pixelStack.push(new PixelRegion(currentPix.getX()-1, currentPix.getY()-1));
                }
            }
            //check pixel directly above
            if(currentPix.checkAdjPix(1) == false)
            {             
                colorArray = getPixelArray(picture[currentPix.getY()][currentPix.getX()-1]);
                color = getPixels(colorArray);
                currentPix.setAdjPix(1, true);
                if(color == rgb)
                {
                    picture[currentPix.getY()][currentPix.getX()-1] = getPixels(black);
                    pixelStack.push(new PixelRegion(currentPix.getX()-1, currentPix.getY()));
                }
            }
            //check pixel to top right
            if(currentPix.checkAdjPix(2) == false)
            {                                
                colorArray = getPixelArray(picture[currentPix.getY()+1][currentPix.getX()-1]);
                color = getPixels(colorArray);
                currentPix.setAdjPix(2, true);
                if(color == rgb)
                {
                    picture[currentPix.getY()+1][currentPix.getX()-1] = getPixels(black);
                    pixelStack.push(new PixelRegion(currentPix.getX()-1, currentPix.getY()+1));
            ...
            if(currentPix.checkAdjPix(3) == false)
            {                
                colorArray = getPixelArray(picture[currentPix.getY()+1][currentPix.getX()]);
                color = getPixels(colorArray);
                currentPix.setAdjPix(3, true);
                if(color == rgb)
                {
                    picture[currentPix.getY()+1][currentPix.getX()] = getPixels(black);
                    pixelStack.push(new PixelRegion(currentPix.getX(), currentPix.getY()+1));
                }
            }
            //check pixel to bottom right
            if(currentPix.checkAdjPix(4) == false)
            {                
                colorArray = getPixelArray(picture[currentPix.getY()+1][currentPix.getX()+1]);
                color = getPixels(colorArray);
                currentPix.setAdjPix(4, true);
                if(color == rgb)
                {
                    picture[currentPix.getY()+1][currentPix.getX()+1] = getPixels(black);
                    pixelStack.push(new PixelRegion(currentPix.getX()+1, currentPix.getY()+1));
            ...
            //check pixel directly below
            if(currentPix.checkAdjPix(5) == false)
            {                
                colorArray = getPixelArray(picture[currentPix.getY()][currentPix.getX()+1]);
                color = getPixels(colorArray);
                currentPix.setAdjPix(5, true);
                if(color == rgb)
                {
                    picture[currentPix.getY()][currentPix.getX()+1] = getPixels(black);
                    pixelStack.push(new PixelRegion(currentPix.getX()+1, currentPix.getY()));
            ...
            //check pixel to bottom left
            if(currentPix.checkAdjPix(6) == false)
            {                
                colorArray = getPixelArray(picture[currentPix.getY()-1][currentPix.getX()+1]);
                color = getPixels(colorArray);
                currentPix.setAdjPix(6, true);
                if(color == rgb)
                {
                    picture[currentPix.getY()-1][currentPix.getX()+1] = getPixels(black);
                    pixelStack.push(new PixelRegion(currentPix.getX()+1, currentPix.getY()-1));
            ...
            if(currentPix.checkAdjPix(7) == false)
            {                
                colorArray = getPixelArray(picture[currentPix.getY()-1][currentPix.getX()]);
                color = getPixels(colorArray);
                currentPix.setAdjPix(7, true);
                if(color == rgb)
                {
                    picture[currentPix.getY()-1][currentPix.getX()] = getPixels(black);
                    pixelStack.push(new PixelRegion(currentPix.getX(), currentPix.getY()-1));
